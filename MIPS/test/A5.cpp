...
// First pass: Collect all label positions
   ...
   void collectLabelPositions(const std::string &assemblyCode) {
    ...
    while (std::getline(iss, line)) {
        ...
        if (line.back() == ':') {
            ...
            // Increment only for actual instructions
            instruction_index++;
        }
    }
}
    ...
    //     int label_index = label_positions[label];
    //     int branch_offset = label_index - current_instruction_index;
  ...
  if (opcode == "beq" || opcode == "bneq") {
    std::string inst_opcode = i_type_opcodes[opcode];
    std::string rs = registers[parts[1]]; // First register
    std::string rt = registers[parts[2]]; // Second register

    std::string label = parts[3];
    if (std::all_of(label.begin(), label.end(), ::isdigit)) {
        // Immediate offset branch (rare case)
        int immediate_value = std::stoi(label);
        std::string immediate = decimalToBinary(immediate_value, 8);
        current_instruction_index++; // Increment the instruction index
        return inst_opcode + rs + rt + immediate;
    }
    ...
    // Calculate offset
    int label_index = label_positions[label]; // Position of the label
    int branch_offset = (label_index - current_instruction_index)/2;

    // Debugging output
    std::cout << "Processing BEQ/BNEQ:"
              << "\n  Current Index: " << current_instruction_index
              << "\n  Label Position: " << label_index
              << "\n  Calculated Offset: " << branch_offset << std::endl;

    // Ensure the offset is in range (-128 to 127 for 8-bit signed value)
    if (branch_offset < -128 || branch_offset > 127) {
        std::cerr << "Error: Branch offset out of range for label '" << label << "'" << std::endl;
        return "Invalid instruction";
    }

    // Convert offset to binary and increment index
    std::string immediate = decimalToBinary(branch_offset, 8);
...
// };
std::vector<std::string> convertProgram(const std::string &assemblyCode) {
    collectLabelPositions(assemblyCode); // First pass to collect labels
    current_instruction_index = 0; // Reset instruction index for second pass
